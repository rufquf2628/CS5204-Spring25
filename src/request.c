...
void request_error(int fd, char *cause, char *errnum, char *shortmsg, char *longmsg) {
    ...
    assert(snprintf(body, MAXBUF,
	    ...
	    "  <p>%s: %s</p>\r\n"
	    "</body>\r\n"
	    "</html>\r\n", errnum, shortmsg, longmsg, cause) > 0
    );
    
    // Write out the header information for this response
    assert(snprintf(buf, MAXBUF, "HTTP/1.0 %s %s\r\n", errnum, shortmsg) > 0);
    write_or_die(fd, buf, strlen(buf));
    
    assert(snprintf(buf, MAXBUF, "Content-Type: text/html\r\n") > 0);
    write_or_die(fd, buf, strlen(buf));
    
    assert(snprintf(buf, MAXBUF, "Content-Length: %lu\r\n\r\n", strlen(body)) > 0);
    write_or_die(fd, buf, strlen(buf));
